...
export const Bitcoin3Page: FC = () => {
  ...
  return (
    <PageTemplate title={t("nav.tree.blockchain")} showChapterNav={false}>
      <BlockReader chapterId={ROUTE_NAME.Bitcoin_3}>
        <Block>
          <ChapterPrelude marginBottom="1.5rem">
            {fr ? (
              <>
                Une blockchain, c'est une chaîne de blocs. Littéralement. Le mot dit ce qu'il fait.
                La vraie question, ce n'est pas « qu'est-ce que c'est ? » : c'est pourquoi c'est si
        ...
        <Block>
          <Callout title={fr ? "Dissection d'un bloc" : "Dissecting a block"}>
            <p>
              {fr ? (
                <>
                  Un bloc se compose de deux parties : un <HighlightText>en-tête</HighlightText> et
                  un <HighlightText>corps</HighlightText>.
                </>
              ) : (
                <>
            ...
            <ol>
              <li>
                {fr ? (
                  ...
                  <span>
                    <HighlightText>Previous block hash</HighlightText> (the fingerprint that links
                    to the block before)
                  </span>
                )}
              </li>
              ...
              <li>
                {fr ? (
                  ...
                  <span>
                    <HighlightText>Merkle root</HighlightText> (a single fingerprint that sums up
              ...
              <li>
                {fr ? (
                  ...
                  <span>
                    <HighlightText>Nonce</HighlightText> (a number used by miners, which we'll get
        ...
        <Block kind="tool">
          {({ markComplete }) => (
            <>
              <p>
                {fr ? (
                  <>
                    ...
                    <i>« Et cette colle mathématique, qu'est-ce que c'est concrètement ? »</i>{" "}
                    <br />
              ...
              <Callout
                title={fr ? "La colle mathématique : Le Hachage" : "The mathematical glue: hashing"}
              >
                <p>
                  {fr ? (
                    <>
                      Pour lier les blocs entre eux, le protocole Bitcoin utilise une fonction de
                      hachage. Le nom fiche un peu la trouille, mais le principe est en réalité
                      ultra simple.
                    </>
                  ) : (
                    <>
                      To link blocks together, the Bitcoin protocol uses a hash function. The name
                      sounds scary, but the idea behind it is dead simple.
                    </>
                  )}
                </p>
                <p>
                  {fr ? (
                    <>
                      Imagine une machine à broyer : tu y jettes n'importe quelle donnée (un mot, un
                      livre entier, une image) et la machine te ressort une empreinte numérique de
                      taille fixe.
                    </>
                  ) : (
                    <>
                      Picture a meat grinder: you toss in any data you want (a word, an entire book,
                      an image) and the machine spits out a digital fingerprint of fixed size.
                    </>
                  )}
                </p>
                <p>
                ...
                <HashDemo onComplete={markComplete} />
                <p>
                  {fr ? (
                    <>
                      ...
                      . Comme un apprenti boucher. Magnifique. Bitcoin, lui, en fait deux à la suite
                      : il hache l'en-tête du bloc, puis il hache le résultat. Un choix historique
                      de Satoshi pour renforcer la résistance à certaines attaques mathématiques. On
                      ne s'attarde pas là-dessus, mais voici un lien pour les plus curieux :{" "}
                    ...
                    <>
                      ...
                      hash. Like a butcher's apprentice. Beautiful. Bitcoin does it twice in a row:
                      it hashes the block header, then hashes the result. A historical design choice
                      by Satoshi to strengthen resistance against certain mathematical attacks. We
                      won't dwell on it here, but here's a link for the curious:{" "}
                      <Reference href="https://github.com/bitcoinbook/bitcoinbook/blob/develop/ch04_keys.adoc">
          ...
          {({ markComplete }) => (
            <>
              <p>
                {fr ? (
                  <>
                    ...
                    produisant une empreinte appelée « hash du bloc ». Cette empreinte sert
                    d'identifiant - et tu vas voir au{" "}
                    <Reference to={ROUTE_NAME.Bitcoin_4}>prochain chapitre</Reference> qu'elle sert
                    aussi à bien autre chose. Ce hash est ensuite repris dans l'en-tête du bloc
                    suivant.
                  </>
                ) : (
                  <>
                    This double hash is applied to the block's header (and only to its header),
                    producing a fingerprint called the "block hash". That fingerprint serves as an
                    identifier - and you'll see in the{" "}
                    <Reference to={ROUTE_NAME.Bitcoin_4}>next chapter</Reference> that it has a lot
                    more going on than just that. This hash is then carried into the header of the
                    next block.
                  </>
                )}
              </p>
              ...
              <p id="blockchainChainVisual-start">
                {fr ? (
                  <>
                    À toi de jouer : voici un bloc, tout seul. Ajoute-en un autre derrière. Tu vas
                    voir ce qui les lie.
                  </>
                ) : (
                  <>
              ...
              <BlockchainChainVisual
                resetScrollTargetId="blockchainChainVisual-start"
        ...
        <Block last>
          <p>
            {fr ? (
              <>
                La blockchain est donc un registre quasi impossible à falsifier en pratique, dont
                chaque page est scellée par la suivante. Mais un registre solide ne suffit pas :
                encore faut-il décider qui a le droit d'y écrire et à quel prix.
              </>
            ) : (
              <>
                The blockchain is therefore a ledger that's practically impossible to falsify, where
                each page is sealed by the next one. But a solid ledger isn't enough: we still need
                to decide who is allowed to write in it, and at what cost.
              </>
            )}
          </p>
          <p>
            {fr ? (
              <>
                Car ce fameux <i>nonce</i>, croisé dans chaque bloc, n'est pas là par hasard. Il est
                la clé d'une compétition mondiale entre mineurs, où la puissance de calcul remplace
                la confiance, et où le hash que tu viens de découvrir joue un rôle central.
              </>
            ) : (
              <>
                Because that <i>nonce</i>, the one you ran into in every block, isn't there by
                accident. It's the key to a worldwide race between miners, where computing power
                replaces trust, and where the hash you just discovered plays a central role.
              </>
            )}
          </p>
          <p>

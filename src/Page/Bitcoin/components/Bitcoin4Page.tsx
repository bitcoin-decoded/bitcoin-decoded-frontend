      ...
      prelude={
        fr ? (
          <>
            ...
            ). Vu de loin, ça paraît absurde. Vu de près, c'est exactement le contraire : c'est la
            seule manière connue de se mettre d'accord à plusieurs millions d'inconnus, sur un
            réseau ouvert à tous, sans chef et sans confiance. Ce chapitre explique pourquoi.
          </>
        ) : (
          <>
            ...
            From far away, it sounds absurd. Up close, it's the exact opposite: it's the only known
            way for millions of strangers to agree on the same ledger, on an open network, with no
    ...
    >
      <p>
        {fr ? (
          ...
          <>
            Picture a global network where nobody knows anyone, and nobody trusts anyone. And yet
            everyone has to agree on a single version of the ledger.
          </>
        )}
      </p>
      <ul>
      ...
      <p>
        {fr ? (
          ...
          <>
            In a centralized system, the answer is easy: an authority decides. But here, there is no
            authority.
          </>
        )}
      </p>
      ...
      <p>
        {fr ? (
          ...
          <>
            ...
            , formulated by Lamport in 1982. Bitcoin is its first solution deployed at scale on an
      ...
      <Callout title={fr ? "Memory Pool ou salle d'attente" : "Memory Pool: the waiting room"}>
        <p>
          {fr ? (
            ...
            <>
              When a transaction is submitted to the Bitcoin network, it isn't validated right away.
              It lands in a waiting room called the "mempool".
            </>
          )}
        </p>
        ...
        <p>
          {fr ? (
            ...
            <>
              Both can travel through the network in parallel. At this point, there's no consensus
              yet.
            </>
          )}
        </p>
      ...
      <p>
        {fr ? (
          <>
            ...
            La réponse est simple et... un peu darwinienne : le premier à trouver la solution gagne.
          </>
        ) : (
          <>
            But here's the catch: anyone can propose a block. So how do we decide who gets to write
            the next page? <br />
      ...
      <p>
        {fr ? (
          ...
          <>
            And here's where you'll come at me: "What happens if two miners find a valid hash at the
            exact same time?" <br />
        ...
        <p>
          {fr ? (
            <>
              ...
              converge vers la chaîne avec le plus de travail cumulé - c'est pas forcément la chaîne
              la plus longue en nombre de blocs, c'est celle dont la difficulté totale est la plus
              élevée (les deux coïncident presque toujours).
            </>
          ) : (
            <>
              ...
              blocks, but the one with the highest total difficulty (which is almost always the same
              thing).
            </>
          )}
        </p>
      ...
      >
        ...
        <p>
          {fr ? (
            ...
            <>
              It readjusts every 2016 blocks (~2 weeks) so that one block is mined every 10 minutes
              on average.
            </>
          )}
        </p>
      ...
      <p>
        {fr ? (
          <>
            Voilà. La preuve de travail accomplit deux prouesses d'un coup : elle rend les conflits
            économiquement ruineux, et elle transforme l'énergie en confiance.
          </>
        ) : (
          <>
            That's it. Proof of Work pulls off two feats at once: it makes conflicts economically
            ruinous, and it turns energy into trust.
          </>
        )}
      </p>

      ...
      <BlockReader chapterId={ROUTE_NAME.Bitcoin_6}>
            ...
            <>
              ...
              >
                <p>
                  {fr ? (
                    <>
                      Dans une banque, la réponse est évidente : ton compte affiche un solde, tu
                      envoies de l'argent, et le système met à jour les chiffres.
                    </>
                  ) : (
                    <>
                      In a bank, the answer is obvious: your account shows a balance, you send
              ...
              <TransactionModelComparison mode="bank" onComplete={markComplete} />
            </>
          )}
        </Block>
          ...
          {({ markComplete }) => (
            <>
              ...
              <p>
                {fr ? (
                  <>
                    Quand tu dis « je vais envoyer 1 bitcoin à mon pote » (j'aimerais bien être ton
                    ami), tu imagines peut-être un objet numérique qui quitte ton portefeuille pour
                    rejoindre celui de quelqu'un d'autre. Eh bien, pas tout à fait.
                  </>
                ) : (
                  <>
                    When you say "I'm going to send 1 bitcoin to a friend" (wish I had friends like
                    that), you might picture a digital object leaving your wallet and arriving in
                    someone else's. Well, not quite.
                  </>
                )}
              </p>
              <p>
              ...
              <p>
                {fr ? (
                  <>
                    ...
                    portefeuille de Nicolas ne contient pas un nombre, mais des <i>pièces</i> de
                    valeurs fixes (0.8, 1, 0.5 et 2), chacune{" "}
                    <HighlightText>indivisible</HighlightText>. Pour payer, il faut piocher dans ces
                    pièces et les combiner.
                  </>
                ) : (
                  <>
              ...
              <UTXOTransactionBuilder lockedAmount="1.3" onComplete={markComplete} />
            </>
          )}
        </Block>
        ...
        <Block>
          <p>
            {fr ? (
              <>
                Tu viens de le sentir : on ne coupe pas une pièce en deux. Pour atteindre un total
                de 1.3 BTC, tu assembles des pièces entières (par exemple : 0,8 BTC + 1 BTC), et
                quand le compte n'est pas juste, le surplus te revient en monnaie.
              </>
            ) : (
              <>
                You just felt it: you can't cut a coin in half. To reach a total of 1.3 BTC, you
                assemble whole coins (for example: 0.8 BTC + 1 BTC), and when the amount isn't
                exact, the surplus comes back to you as change.
              </>
            )}
          </p>
          <p>
            {fr ? (
              <>
                ...
                transaction Bitcoin n'est pas un virement : elle consomme des pièces existantes et
                en recrée de nouvelles. Tu peux voir un <HighlightText>UTXO</HighlightText> comme
                une pièce rangée dans ton portefeuille : indivisible, prête à être dépensée.
              </>
            ) : (
              <>
                These "coins" have a name: a <i>UTXO</i> (<i>Unspent Transaction Output</i>). A
                Bitcoin transaction is not a wire transfer: it consumes existing coins and creates
                new ones. Think of a <HighlightText>UTXO</HighlightText> as a coin sitting in your
                wallet: indivisible, ready to be spent.
              </>
            )}
          </p>
          ...
          {({ markComplete }) => (
            <>
              <p>
                {fr ? (
                  <>
                    Ton portefeuille ne stocke aucun bitcoin. Étrange, hein ? Plus précisément : il
                    ne stocke pas tes bitcoins comme des fichiers sont stockés sur ton ordi. Il
                    stocke en réalité{" "}
                    <HighlightText>
                      les clés privées qui te permettent de dépenser tes UTXO.
                    </HighlightText>
                  </>
                ) : (
                  <>
                    Your wallet stores zero bitcoins. Weird, right? More precisely: it doesn't store
                    your bitcoins the way files are stored on your computer. What it actually stores
                    is{" "}
              ...
              <TransactionModelComparison mode="bitcoin" onComplete={markComplete} />
            </>
          )}
        </Block>
        ...
        <Block kind="tool">
          {({ markComplete }) => (
            <>
              ...
              <Quiz {...getQuizDataUtxo(language)} onCorrectAnswer={markComplete} />
            </>
          )}
        </Block>
          ...
          <p>
            {fr ? (
              ...
              <>
                ...
                <i>how does the network know you actually have the right to spend those outputs?</i>{" "}
                And{" "}
                <i>
                  how do you prove that ownership without relying on a bank, an account, or a
                  central authority?
                </i>
              </>
            )}

      ...
      <BlockReader chapterId={ROUTE_NAME.MoneyLaws_5}>
        <Block>
          <ChapterPrelude marginBottom="1.5rem">
            {fr ? (
              ...
              <>
                In 2008, central banks injected trillions. Many feared consumer prices would
                explode. They didn't. So printing money doesn't cause inflation? Wrong answer. Wrong
                method, mostly.
              </>
            )}
          </ChapterPrelude>
          <p>
            {fr ? (
              <>
                ...
                <Reference to={ROUTE_NAME.MoneyLaws_4}>doctrine socialiste</Reference> s'effondre
                par manque d'information. Mais alors, comment les économistes font-ils pour traiter
                cette information ? C'est là que les chemins se séparent.
              </>
            ) : (
              <>
                In the previous chapter, you saw how the{" "}
                <Reference to={ROUTE_NAME.MoneyLaws_4}>socialist doctrine</Reference> collapses from
                a lack of information. But then, how do economists actually process that
                information? This is where the paths split.
              </>
            )}
          </p>
        </Block>
          ...
          <p>
            {fr ? (
              ...
              <>
                ...
                , an economist of the Austrian school, called out what he named scientism: the urge
                to apply hard-science methods (physics, chemistry, that sort of thing) to human
                action.
              </>
            )}
          </p>
            ...
            <li>
              {fr ? (
                <span>
                  ...
                  exemple, si les impôts baissent et que la croissance se met à monter, est-ce à
                  cause des impôts ou d'une nouvelle technologie apparue au même moment ? Impossible
                  de trancher clairement.
                </span>
              ) : (
                <span>
                  Society isn't a lab. You can't isolate a single variable. For example, if taxes
                  drop and growth picks up, is it because of the tax cut, or because of some new
                  technology that showed up at the same time? Impossible to settle cleanly.
                </span>
              )}
            </li>
            <li>
              {fr ? (
                <span>
                  L'humain apprend. Contrairement à une planète, l'humain change son comportement
                  s'il connaît une théorie. Les données du passé n'enferment pas le futur dans un
                  modèle stable et immuable.
                </span>
              ) : (
                <span>
          ...
          {({ markComplete }) => (
            <>
              <p>
                {fr ? (
                  <>
                    Toute la pensée autrichienne repose sur une seule brique, une vérité de base.
                    C'est ce qu'on appelle l'axiome praxéologique (de la{" "}
                  ...
                  <>
                    All of Austrian thought rests on a single building block, a foundational truth.
                    It's called the praxeological axiom (from{" "}
                    <Reference href="https://en.wikipedia.org/wiki/Praxeology">
                      praxeology
                    </Reference>
              ...
              <Quote>
                {fr ? "L'être humain agit dans un but." : "Human beings act with a purpose."}
              </Quote>
              <p>
                {fr
              ...
              <Quiz {...getQuizDataAustrianAxiom(language)} onCorrectAnswer={markComplete} />
            </>
          )}
        </Block>
        ...
        <Block>
          {fr ? (
            <>
              ...
              cela signifie que nos ressources sont rares (le temps notamment), que nous faisons des
              choix, et que nous avons une préférence temporelle (à valeur égale, on préfère tout de
              suite plutôt que plus tard).
            </>
          ) : (
            <>
              Everything else follows from this axiom: since we act to reach a goal, it means our
              resources are scarce (time, above all), that we make choices, and that we have a time
              preference (all else equal, we'd rather have it now than later).
            </>
          )}
        </Block>
        ...
        <Block kind="tool">
          {({ markComplete }) => (
            <>
              ...
              <DebateArena
                items={getKeynesianVsAustrian(language)}
                requiredExplored={2}
        ...
        <Block last>
          <p>
            {fr ? (
              <>
                L'école autrichienne ne cherche pas à prédire des choses du style « le PIB montera
                de 1.2 % ». Elle établit des lois qualitatives et universelles.
                <br />
                Par exemple : « la manipulation artificielle des taux d'intérêt provoque
                systématiquement une mauvaise allocation du capital ». Pour un autrichien, ce n'est
                pas une opinion à débattre : c'est une conséquence logique du raisonnement. On ne le
                prouve pas avec des données, on le déduit.
              </>
            ) : (
              <>
                The Austrian school isn't trying to predict things like "GDP will rise by 1.2%". It
                lays out qualitative, universal laws.
                <br />
                For instance: "the artificial manipulation of interest rates systematically leads to
                a misallocation of capital". For an Austrian, this isn't an opinion up for debate,
                it's a logical consequence of the reasoning. You don't prove it with data, you
                deduce it.
              </>
            )}
          </p>
          <p>
            {fr ? (
              <>
                On boucle la boucle ici. Les conclusions des chapitres précédents ne sortent pas
                d'une préférence politique. Ce sont des conséquences logiques tirées de la nature
                même de l'être humain. Libre à toi d'en tirer, ensuite, les conclusions politiques
                que tu veux.
              </>
            ) : (
              <>
                ...
                human being. What political conclusions you then draw from them, that's entirely up
                to you.
              </>
            )}
          </p>
          <p>
            ...
            <Reference to={ROUTE_NAME.MoneyLaws_6}>

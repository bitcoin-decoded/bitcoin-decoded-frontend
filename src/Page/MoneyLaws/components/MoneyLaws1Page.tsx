...
export const MoneyLaws1Page: FC = () => {
  ...
  return (
    <PageTemplate title={t("nav.tree.whatIsMoney")} showChapterNav={false}>
      <BlockReader chapterId={ROUTE_NAME.MoneyLaws_1}>
        <Block>
          <ChapterPrelude marginBottom="1.5rem">
            {fr ? (
              <>
                Une monnaie ne sert à rien toute seule. Elle ne vaut quelque chose que parce qu'un
                réseau d'utilisateurs l'accepte. Parfois par habitude, parfois par contrainte
                légale, parfois par choix. Toute la question, depuis 5000 ans, c'est : pourquoi on
                adopte cette monnaie-ci, et qu'on abandonne celle-là ?
              </>
            ) : (
              <>
                A currency is useless on its own. It only has value because a network of users
                agrees to use it. Sometimes out of habit, sometimes by legal mandate, sometimes by
                choice. The real question, for 5,000 years now, has been: why do we adopt this
          ...
          <p>
            {fr ? (
              ...
              <>
                If I ask you what's in your wallet, you'll probably answer (after reminding me it's
                none of my business): <i>"money"</i>.
              </>
            )}
          </p>
          <p>
            {fr ? (
              <>
                Pourtant, seul sur une île déserte avec un million d'euros, tu serais pauvre.
                Pourquoi ? Parce que les billets ne se mangent pas. La monnaie ne vaut rien sans les
                autres : elle est avant tout{" "}
                <HighlightText>une technologie de collaboration</HighlightText>.
              </>
            ) : (
              <>
                And yet, stranded alone on a desert island with a million dollars, you'd be poor.
                Why? Because you can't eat banknotes. Money is worthless without other people: above
                all, it's <HighlightText>a collaboration technology</HighlightText>.
              </>
            )}
          </p>
        </Block>
        <Block>
          <Callout title={fr ? "Le nombre de Dunbar" : "Dunbar's number"}>
            ...
            <p>
              {fr ? (
                <>
                  Notre cerveau n'est pas câblé pour gérer un nombre infini de relations. Dans un
                  petit groupe, on se connaît, on se fait confiance, on s'arrange :{" "}
                  <i>« je t'aide pour ta hutte, tu m'aideras pour ma récolte »</i>. Pas besoin de
                  monnaie.
                </>
              ) : (
                <>
                  Our brain isn't wired to handle an infinite number of relationships. In a small
                  group, everyone knows each other, trust is built in, things get arranged:{" "}
            ...
            <p>
              {fr ? (
                ...
                <>
                  ...
                  is the estimated cognitive limit of our brain for keeping stable relationships
                  going.
                </>
              )}
            </p>
            <p>
              {fr ? (
                <>
                  Ce n'est pas une frontière magique au-dessus de laquelle la monnaie « apparaît »,
                  mais plutôt le seuil où la mémoire individuelle commence à coincer. Au-delà, il
                  faut un <HighlightText>système externe</HighlightText> pour tracer qui doit quoi à
                  qui.
                </>
              ) : (
                <>
            ...
            <p>
              {fr ? (
                <>
                  ...
                  parfois d'un crédit accordé. Et le droit de réclamer un service équivalent en
                  retour.
                </>
              ) : (
                <>
                  Money then steps in: it becomes <HighlightText>a collective memory</HighlightText>
                  . Owning money means holding a form of social recognition, usually the trace of
                  work you've done, sometimes credit extended to you. And the right to claim an
        ...
        <Block>
          <p>
            {fr ? (
              <>
                La monnaie a un job précis : transférer la valeur de notre travail dans l'espace,
                mais surtout dans le temps.
              </>
            ) : (
              <>
                Money has one specific job: to transfer the value of our work across space, but
                above all across time.
              </>
            )}
          </p>
          <p>
            {fr ? (
              <>
                ...
                stockons la valeur de notre travail aujourd'hui pour pouvoir la restituer plus tard.
                On peut imaginer la monnaie comme une batterie stockant de l'énergie. Mais attention
                : pour qu'une batterie soit utile, il ne faut pas qu'elle se vide toute seule !
              </>
            ) : (
              <>
                Since we have no idea what tomorrow holds (health issues, setbacks, opportunities),
                we store the value of our work today so we can pull it back out later. Think of
                money as a battery storing energy. But careful: for a battery to be useful, it can't
                drain on its own!
              </>
            )}
          </p>
        ...
        <Block kind="tool">
          {({ markComplete }) => (
            <>
              ...
              <MonetaryGallery onComplete={markComplete} />
            </>
          )}
        </Block>
        <Block>
          <p>
            {fr ? (
              ...
              <>
                The verdict from this gallery is clear-cut: the moment a currency loses its
                hardness, it stops working as a store of value and inevitably ends up vanishing.
              </>
            )}
          </p>
          <p>
            {fr ? (
              <>
                C'est une régularité historique frappante : à long terme, les utilisateurs
                délaissent les batteries qui fuient pour celles qui tiennent.{" "}
                <HighlightText>
                  Une monnaie facile à diluer se vide toute seule. Une monnaie durablement dure
                  devient mécaniquement de plus en plus rare, et protège ceux qui l'utilisent
                </HighlightText>
                .
              </>
            ) : (
              <>
                It's a striking historical pattern: over the long run, users abandon the batteries
                that leak for the ones that hold.{" "}
                <HighlightText>
        ...
        <Block last>
          ...
          <p>
            {fr ? (
              <>
                Mais attention : quand on utilise une monnaie « facile à créer », on fausse le prix
                du temps et de l'effort. On envoie de faux signaux à toute la société, ce qui crée
                des phases d'euphorie artificielle suivies de krachs violents.
              </>
            ) : (
              <>
                But careful: when we use a currency that's "easy to create," we distort the price of
                time and effort. We send false signals to the entire society, which creates phases
                of artificial euphoria followed by violent crashes.
              </>
            )}
          </p>
          <p>
          ...
          <p>
            {fr ? (
              <>
                ...
                <Reference to={ROUTE_NAME.MoneyLaws_2}>cycles économiques</Reference> que nous
                subissons ? On y va.
              </>
            ) : (
              <>
                Ready to see how this lack of hardness mechanically manufactures the{" "}
                <Reference to={ROUTE_NAME.MoneyLaws_2}>economic cycles</Reference> we all endure?
                Let's go.
              </>
            )}
          </p>

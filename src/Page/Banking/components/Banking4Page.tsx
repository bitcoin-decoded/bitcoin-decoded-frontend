...
export const Banking4Page: FC = () => {
  ...
  return (
    <PageTemplate title={t("nav.tree.brokenEngine")}>
          ...
          <ChapterPrelude marginBottom="1.5rem">
            {fr ? (
              <>
                ...
                de financer un vrai projet, mais déroule le tapis rouge dès qu'il s'agit d'un crédit
                immobilier ou d'un placement ? Ce n'est pas (forcément) ton dossier. C'est son
                moteur qui tourne au ralenti. Et la Banque Centrale y est pour beaucoup.
              </>
            ) : (
              <>
                Ever wondered why your banker gives you a funny look when you mention financing a
                real project, but rolls out the red carpet the moment it's a mortgage or an
                investment product? It's not (necessarily) you. It's their engine running on fumes.
                And the Central Bank has a lot to do with it.
              </>
            )}
          </ChapterPrelude>
          <p>
            {fr ? (
              <>
                Jusqu'ici, la Banque Centrale a volé au secours des banques commerciales en inondant
                le système de liquidités. Le problème c'est qu'en faisant ça, elle a enrayé le
                moteur.
              </>
            ) : (
              <>
                Up to now, the Central Bank has come to the rescue of commercial banks by flooding
                the system with liquidity. The problem is that, in doing so, it jammed the engine.
              </>
            )}
          </p>
          <p>
            ...
            <ol>
              <li>
                {fr ? (
                  ...
                  <>
                    It <i>borrows</i> money short-term, at a rate that's usually very low (close to
                    0%)
                  </>
                )}
              </li>
            ...
            <p>
              {fr ? (
                <>
                  ...
                  elle-même emprunter à court terme pour tourner, à un taux très bas voire quasi
                  nul. Son profit, c'est l'écart entre les 3% que paie Nicolas et ce taux-là.
                </>
              ) : (
                <>
                  Remember <i>Nicolas</i>: his bank created the €200,000 in a single click. But
                  lending is never free - Nicolas has to be creditworthy, and the bank itself has to
                  borrow short-term to keep running, at a very low, near-zero rate. Its profit is
                  the gap between the 3% Nicolas pays and that rate.
                </>
              )}
            </p>
          </Callout>
          ...
          {({ markComplete }) => (
            <>
              <p>
                {fr ? (
                  <>
                    En achetant massivement des obligations, la Banque Centrale fait mécaniquement
                    monter leur prix (la loi de <i>l'offre et de la demande</i>). Et cette montée du
                    prix des obligations fait simultanément baisser leur taux d'intérêt.
                  </>
                ) : (
                  <>
                    By buying bonds on a massive scale, the Central Bank mechanically pushes their
                    price up (the law of <i>supply and demand</i>). And as bond prices rise, their
                    interest rate falls at the same time.
                  </>
                )}
              </p>
              <p>
              ...
              <p>
                {fr ? (
                  <>
                    T'es maintenant une banque centrale et tu vas réaliser une opération
                    d'assouplissement quantitatif. Observe les effets que cela provoque sur les
                    obligations.
                  </>
                ) : (
                  <>
                    You're now a central bank, and you're about to run a quantitative easing
                    operation. Watch what it does to bonds.
                  </>
                )}
              </p>
              <QESimulator onComplete={markComplete} />
            </>
          )}
        </Block>
            ...
            <>
              <p>
                {fr ? (
                  ...
                  <>
                    At which point you'll probably go:{" "}
                    <i>"OK, great... so what? Is it serious, Doctor?"</i> and I'll tell you straight
                    away: "Yes, because it breaks how commercial banks make their money."
                  </>
                )}
              </p>
              <p>
              ...
              <YieldCurveSimulator onComplete={markComplete} />
            </>
          )}
        </Block>
          ...
          <p>
            {fr ? (
              ...
              <>
                ...
                (creating value, launching projects by funding entrepreneurs, hiring) becomes far
                less profitable for banks. Too much risk for such a thin margin.
              </>
            )}
          </p>
          <p>
            {fr ? (
              <>
                Mais les banques doivent survivre ! Elles vont donc opérer un virage stratégique
                majeur. Elles vont délaisser la production pour se concentrer massivement sur le
                financement de l'<i>Économie Patrimoniale</i>.
              </>
            ) : (
              <>
                But banks have to survive! So they pull off a major strategic turn. They walk away
                from production and pour their efforts into financing the <i>Asset Economy</i>{" "}
                instead.
              </>
            )}
          </p>

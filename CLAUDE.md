...
- **Exploration obligatoire** (compteur « x/N exploré » façon `KeySignatureTrio`) : le composant déclenche `onComplete` au franchissement du seuil. Primitives partagées : `ExploredCounter` (`Design/components`) + `useExplorationGate` (`Design/hooks`, set de N items distincts). Cas pilotes : `MonetaryGallery` (3 monnaies dépliées, signal via `IdentityCard onExpand`) ; `CapitalStructureChain` (2 détours remontés, compteur dérivé de `count`).
